...
err_report() {
  ...
  curl -X POST -H 'Content-type: application/json' --data '{"text":":rage:\n'`uname -n`' TDBUPDATE-OK"}' https://hooks.slack.com/services/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
  exit 0
}
trap err_report ERR
...
if ($new_nl - $old_nl < -500) then
    ...
    curl -X POST -H 'Content-type: application/json' --data '{"text":":rage:\n'`uname -n`' AUTOPSISUM-FAILED"}' https://hooks.slack.com/services/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
    exit
endif
